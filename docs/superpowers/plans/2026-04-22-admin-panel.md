...
**Goal:** Build a web-based admin panel for managing products, orders, cities, and store settings, protected by a static admin token.

**Architecture:** React 18 + Vite + TypeScript + Tailwind CSS. All API calls go to the backend from Plan 1 using an `Authorization: Bearer <ADMIN_SECRET>` header. State is managed with TanStack Query (server) + React Hook Form (forms). Authentication is a simple local token stored in localStorage.
...
│   ├── App.tsx
│   ├── lib/
│   │   ├── api.ts                     # Axios instance with admin token header
│   │   └── queryClient.ts
│   ├── store/
│   │   └── adminStore.ts              # Zustand: admin token
│   ├── hooks/
│   │   ├── useAdminProducts.ts
│   │   ├── useAdminOrders.ts
│   │   ├── useAdminCities.ts
...
**Files:**
- Create: `admin/` directory with Vite + React + Tailwind
...
```bash
git add admin/
git commit -m "feat: scaffold admin panel with Vite + React + Tailwind"
```
...
- Create: `admin/src/store/adminStore.ts`
  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state?.token) setAdminToken(state.token)
      },
    }
  )
)
    ...
    <div className="flex min-h-screen bg-gray-50">
      <aside className="w-56 bg-white border-r flex flex-col">
        <div className="p-5 border-b">
          <h1 className="font-bold text-lg">Lojinha Admin</h1>
        </div>
...
**Files:**
- Create: `admin/src/hooks/useAdminProducts.ts`
- Create: `admin/src/hooks/useAdminOrders.ts`
- Create: `admin/src/hooks/useAdminCities.ts`
- Create: `admin/src/hooks/useAdminSettings.ts`
...
export function useCreateProduct() {
  ...
  return useMutation({
    mutationFn: (data: Partial<AdminProduct>) => api.post('/admin/products', data).then((r) => r.data),
...
export function useAddProductMedia(productId: string) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (data: { type: string; url: string; sortOrder?: number }) =>
      api.post(`/admin/products/${productId}/media`, data).then((r) => r.data),
...
export function useAdminCities() {
  return useQuery<AdminCity[]>({
    ...
    queryFn: async () => (await api.get('/admin/cities')).data,
  })
}
...
export function useCreateCity() {
  ...
  return useMutation({
    mutationFn: (data: { name: string; slug: string; sortOrder?: number }) =>
      api.post('/admin/cities', data).then((r) => r.data),
...
- [ ] **Step 4: Create src/hooks/useAdminSettings.ts**
...
git commit -m "feat: add admin data hooks for products, orders, cities, settings"
```
...
**Files:**
- Create: `admin/src/pages/products/ProductList.tsx`
- Create: `admin/src/pages/products/ProductForm.tsx`
- Create: `admin/src/pages/products/ProductMediaManager.tsx`

- [ ] **Step 1: Create src/pages/products/ProductList.tsx**
            ...
            {products?.map((p) => (
              <tr key={p.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right space-x-2">
                  ...
                  <button
                    onClick={() => {
                      if (confirm(`Desativar "${p.name}"?`)) deleteProduct.mutate(p.id)
                    }}
...
- [ ] **Step 2: Create src/pages/products/ProductForm.tsx**
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-white border rounded-2xl p-6">
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full px-4 py-2.5 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 resize-none"
          />
        </div>
...
- [ ] **Step 3: Create src/pages/products/ProductMediaManager.tsx**
...
export default function ProductMediaManager({ productId }: Props) {
...
```bash
git add admin/src/pages/products/
git commit -m "feat: add admin products list, form, and media manager"
```
...
- Create: `admin/src/pages/orders/OrderList.tsx`
- Create: `admin/src/pages/orders/OrderDetail.tsx`
...
- [ ] **Step 2: Create src/pages/orders/OrderList.tsx**
          ...
          <tbody className="divide-y">
            {orders?.map((o) => (
              <tr key={o.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <button onClick={() => navigate(`/orders/${o.id}`)} className="text-blue-600 hover:underline text-sm">
                    Ver
                  </button>
                </td>
              </tr>
...
- [ ] **Step 3: Create src/pages/orders/OrderDetail.tsx**
    ...
    <div className="p-8 max-w-2xl">
      ...
      <div className="bg-white border rounded-2xl p-6 mb-4 space-y-3">
        <div className="flex justify-between">
          ...
          <span className="font-medium">{order.user.firstName}{order.user.username ? ` (@${order.user.username})` : ''}</span>
        </div>
        <div className="flex justify-between">
        ...
        <div className="bg-white border rounded-2xl p-6">
          ...
          <div className="flex gap-2">
            {nextStatuses.map((s) => (
              ...
              >
                {s === 'in_review' ? 'Em análise' : s === 'confirmed' ? 'Confirmar' : 'Cancelar'}
              </button>
            ))}
          </div>
        </div>
...
```bash
git add admin/src/components/StatusBadge.tsx admin/src/pages/orders/
git commit -m "feat: add admin orders list and detail with status update"
```

---

## Task 7: Cities + Settings Pages

**Files:**
- Create: `admin/src/pages/cities/CityList.tsx`
- Create: `admin/src/pages/cities/CityForm.tsx`
- Create: `admin/src/pages/settings/StoreSettings.tsx`

- [ ] **Step 1: Create src/pages/cities/CityList.tsx**

```typescript
import { useNavigate } from 'react-router-dom'
import { useAdminCities, useUpdateCity } from '@/hooks/useAdminCities'
    ...
    <div className="p-8">
      ...
      <div className="bg-white border rounded-2xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {cities?.map((c) => {
              const toggle = useUpdateCity(c.id)
              return (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right space-x-2">
                    <button
                      onClick={() => navigate(`/cities/${c.id}/edit`)}
                      className="text-blue-600 hover:underline"
                    >
                      Editar
                    </button>
                    <button
                      onClick={() => toggle.mutate({ isActive: !c.isActive })}
...
> Note: `useUpdateCity(c.id)` inside a `.map()` violates React rules of hooks. Refactor `CityList` to use a `CityRow` sub-component so each row calls `useUpdateCity(c.id)` at the top level of its own component:
...
Replace the `.map()` in `CityList` with `cities?.map((c) => <CityRow key={c.id} city={c} />)`.
...
export default function CityForm() {
  ...
  useEffect(() => {
    if (city) reset({ name: city.name, slug: city.slug, sortOrder: city.sortOrder })
  }, [city, reset])
  ...
  return (
    <div className="p-8 max-w-md">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate(-1)} className="text-blue-600">←</button>
        <h1 className="text-2xl font-bold">{isEdit ? 'Editar cidade' : 'Nova cidade'}</h1>
      </div>
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white border rounded-2xl p-6 space-y-4">
        ...
        <div>
          ...
          <input type="number" {...register('sortOrder', { valueAsNumber: true })} className="w-full px-4 py-2.5 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
...
- [ ] **Step 3: Create src/pages/settings/StoreSettings.tsx**
    ...
    <div className="p-8 max-w-xl">
      ...
      <form onSubmit={handleSubmit((data) => update.mutate(data))} className="bg-white border rounded-2xl p-6 space-y-4">
        <div>
          ...
          <input {...register('storeName')} className="w-full px-4 py-2.5 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">URL do logo</label>
          <input {...register('logoUrl')} className="w-full px-4 py-2.5 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Link do suporte Telegram</label>
          <input {...register('supportTelegramUrl')} placeholder="https://t.me/suporte" className="w-full px-4 py-2.5 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Texto de boas-vindas (bot)</label>
          <textarea {...register('welcomeText')} rows={3} className="w-full px-4 py-2.5 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
        </div>
...
```bash
git add admin/src/pages/cities/ admin/src/pages/settings/
git commit -m "feat: add cities and store settings admin pages"
```
...
Enter the ADMIN_SECRET from `.env` — should redirect to dashboard with all nav items visible.
